...
function ListOeuvres({ navigation }) {
  ...
  const handle_delete_oeuvre = (oeuvre) => {
  ...
  return (
    <View style={styles.container}>
      ...
      <FlatList
        data={oeuvres.filter((oeuvre) => oeuvre.auteur === user.email)}
        renderItem={({ item }) => (
          <TouchableOpacity style={styles.cardContainer} onPress={() => navigation.navigate('OeuvreDetail', { oeuvre: item })}>
            ...
            {user && user.email === item.auteur && (
              <View style={styles.buttonContainer}>
                <TouchableOpacity style={styles.deleteButton} onPress={() => handle_delete_oeuvre(item)}>
